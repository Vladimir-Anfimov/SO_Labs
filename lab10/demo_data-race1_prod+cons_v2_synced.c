/*
  Program: demo_data-race1_prod+cons_v2_synced.c (Varianta sincronizată !  ==> nu mai suferă de data-race-uri !!!)
...
void producer_work()
{
    ...
    for(i = 0; i < NR_TEXTE; i++)
    {
        strcpy(map_addr + i*DIM_TEXTE, texte_de_scris[i]);  // Scriem al i-lea text în bufferul partajat de cele două procese
...
void consumer_work()
{
    ...
    for(i = 0; i < NR_TEXTE; i++)
    {
        /* Ideea de sincronizare: așteptăm până când producătorul va scrie al i-lea text (adică până când nu mai "vedem" zerouri la acele adrese...)
           De fapt, conform celor explicate la acest exercițiu rezolvat în suportul de laborator #10, este suficient să așteptăm până când "vedem"
           non-zero doar la penultima adresă, dintre cele 10 adrese consecutive folosite pentru a stoca textul al i-lea.
		*/
		
        /* varianta 1 de sincronizare: așteptare ocupată */
        // do{ } while( *(map_addr + (i+1)*DIM_TEXTE - 2) == 0);
		// I am "spinning" on the CPU, testing repeatedly for zero...

        /* varianta 2 de sincronizare: așteptare blocantă */
        do{ sleep(1); } while( *(map_addr + (i+1)*DIM_TEXTE - 2) == 0);
        ...
        // do{ sleep(0); } while( *(map_addr + (i+1)*DIM_TEXTE - 2) == 0);
		// I am going to the ready state (i.e., I preempt myself, but without an effective sleep), then running and test again for zero...


        strcpy(texte_de_citit[i], map_addr + i*DIM_TEXTE);  // Citim al i-lea text din bufferul partajat de cele două procese
